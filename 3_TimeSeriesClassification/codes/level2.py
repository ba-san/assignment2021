...
def level2(path):
    ...
    with open(path) as f:
        ...
        x = [float(target[i].split('\t')[0]) for i in range(len(target))]
        y = [float(target[i].split('\t')[1]) for i in range(len(target))]
        z = [float(target[i].split('\t')[2]) for i in range(len(target))]
